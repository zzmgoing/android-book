...
**1、查找效率：** HashMap高，ArrayMap略逊于HashMap。  
**2、扩容效率：** ArrayMap高，HashMap每次扩容都是原来的2倍，需要重新计算key的下标将数组成员放到新的位置，ArrayMap长度小于4申请4的长度，小于8申请8的长度，大于8会申请长度的1.5倍空间，并且是直接通过System.arraycopy来进行赋值。  
**3、内存消耗：** HashMap比较消耗内存，ArrayMap能够重复利用因为数据扩容而遗留下来的数组空间,方便下一个ArrayMap的使用，如果频繁使用map，并且数据量都比较小的时候，使用ArrayMap比较节省内存。  
